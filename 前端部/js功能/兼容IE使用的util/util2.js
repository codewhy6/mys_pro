 ...
 * @LastEditTime: 2020-09-27 14:45:33
 */
...
var formatTime = function(date){
  ...
  */
  ...
  var popupReminder = function (content, callback, showCancel = false) {
    ...
    setTimeout(function () {
      var btns = showCancel == false ? '确定' : ['确认', '取消'];
      layer.open({
        content: content,
        btn: btns,
        shadeClose: false,
   ...
   */
  function setPicker(pickerId, arr, keysName, columnS , isShowParent ) {
    columnS = columnS || 1
    isShowParent = isShowParent || false
    var ID = pickerId;
    var pickerArr = [], pickerArrSecond = [],pickerArrThird=[];
    var pickerShop = null; //初始化picker
   ...
   */
  var popupWindowMobile = function (content, type , title , btnsName ) {
    type = type || 1
    title = title || "提醒"
    btnsName = btnsName || ['确定', '取消']
    var htmlStr = "";
    if (type == 1) {
  ...
  var getValue = function (arr, key, value) {
    ...
    var filterValue = arr.find(function(val){
  ...
  /**
      ...
      $("body").on("change", "#upLoadFile", function () {
        var file = $(this)[0].files[0];//获取上传的文件
        var fileName = file.name; //上传的文件名
        var fileIcon = null;
        var point = fileName.lastIndexOf(".");
        ...
        var str = '<p class="evaluation_file_p2">'+
  ...
  /**
    ...
    */
   var timeStamp = function (second_time, type ) {
    type = type || 1
   ...
   * @return: 
   */
  var getBirthAge = function(IDcard, type ) {
    type = type || 1
    IDcard = IDcard.replace(/\s+/g, "");//去除空格
     debugger
    var myDate = new Date();//获取当前时间
    var tYear = myDate.getFullYear();//获取当前年份
    var age = 0;//根据身份证换算年龄
    var birthday = "";//出生日期
    if (type == 1) {
  ...
  * @Date: @Date: 2019-11-20
  */
  var setArticleDetailImgSrc = function (content) {
   ...
   * @Description: //说明：javascript的加法结果会有误差，在两个浮点数相加的时候会比较明显。这个函数返回较为精确的加法结果。
   * @Description://调用：accAdd(arg1,arg2)
   * @Description:  //返回值：arg1加上arg2的精确结果
   * @Author: Cheng Linmao
   * @Date: 2020-07-31 10:44:02
   * @param {NumberLength} NumberLength:number类型,保留几位有效数字
   * @return: 类型 Number
   */
  function accAdd(arg1, arg2, NumberLength) {
   ...
   * @Description://说明：javascript的乘法结果会有误差，在两个浮点数相乘的时候会比较明显。这个函数返回较为精确的乘法结果。
   * @Description://调用：accMul(arg1,arg2)
   * @Description://返回值：arg1乘以arg2的精确结果
   * @Author: Cheng Linmao
   * @Date: 2020-07-31 10:44:02
   * @param {NumberLength} NumberLength:number类型,保留几位有效数字
   * @return:类型 Number
  ...
  function accMul(arg1, arg2, NumberLength ) {
    NumberLength = NumberLength || 2
