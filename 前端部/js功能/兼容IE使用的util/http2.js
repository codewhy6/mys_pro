 ...
 * @fileName: 
 * @Author: longhaiyan
 * @Date: 2020-09-07 14:15:43
 * @LastEditors: longhaiyan
 * @LastEditTime: 2020-09-07 14:16:43
 */
...
var baseeUrl = "http://javatest01.chemguan.com/"; //基础路径 这个需要修改
//var baseeUrl="http://192.168.1.158:9999"//龚
var imgStateUrl = baseeUrl + "projectimg/";//动态图片资源路径
...
function http(url, callback, requireType , data , loading ) {
    requireType = requireType || 'post', data = data || null, loading = loading || true
    if (loading == true) {
        layer.open({
            type: 2,
            shadeClose: false,
...
function uploadFileXun(file,callback) {
    ...
    var uploadFileXunUrl = "club/column/api/mobile/uploadFileXun";
